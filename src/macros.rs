...
///
///
/// # Traits withing the `unsynn!` macro ***(EXPERIMENTAL)***
///
/// The `unsynn!` macro has limited but ergonomic support for defining and implementing
/// traits. The main purpose for this is to be able to define simple markers and accessors so
/// that generic parsers can be made compile-time validated.
///
/// Trait definitions are just passed though. The only simplification here is that they are
/// allowed to end with a `;` instead a `{}`. This signifies simple marker traits that have no
/// methods.
///
/// Trait implementations are either passed through or are a `impl {}` directly following an
/// ADT definition. In the later case most boilerplate is left out and will be autogenerated
/// by the macro.
///
///
/// ## Example
...
// / PLANNED:     trait GenericAccessor<T> { fn get(&self) -> T;}
///
...
// / PLANNED: impl GenericAccessor<bool> for bool {fn get(&self) -> bool {*self}}
/// }
/// ```
#[cfg(doc)]
#[macro_export]
...
macro_rules! unsynn{
    ...
    // TODO: supertraits, generics
    (
        $(#[$attribute:meta])* $pub:vis trait $name:ident;
        $($cont:tt)*
    ) => {
        $(#[$attribute])* $pub trait $name {}
        // next item
        $crate::unsynn!{$($cont)*}
    };
    (
        $(#[$attribute:meta])* $pub:vis trait $name:ident {$($body:tt)*}
        $($cont:tt)*
    ) => {
        $(#[$attribute])* $pub trait $name {$($body)*}
        // next item
        $crate::unsynn!{$($cont)*}
    };
