...
/// that generic parser definitions can be compile-time validated.
///
/// Trait definitions are passed though. One simplification here is that they are allowed to
/// end with a `;` instead a `{}`. This signifies simple marker traits that have no
/// methods. Next the form `trait {Name; ...}` is supported to define a set of simple markers
